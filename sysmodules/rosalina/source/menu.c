...
void menuThreadMain(void)
{
    ...
    Menu rosalinaMenuTmp = {
        Lang11_Get(SID_MENU_ROOT_TITLE),
        {
            ...
            { Lang11_Get(SID_MENU_ROOT_ENTRY_PROCLIST), METHOD, .method = &RosalinaMenu_ProcessList },
            { Lang11_Get(SID_MENU_ROOT_ENTRY_DEBUGOPTS), MENU, .menu = &debuggerMenu },
            { Lang11_Get(SID_MENU_ROOT_ENTRY_SYSTEMCFG), MENU, .menu = &sysconfigMenu },
            { Lang11_Get(SID_MENU_ROOT_ENTRY_MISCELLANEOUS), MENU, .menu = &miscellaneousMenu },
            { Lang11_Get(SID_MENU_ROOT_ENTRY_SAVESETTINGS), METHOD, .method = &RosalinaMenu_SaveSettings },
            { Lang11_Get(SID_MENU_ROOT_ENTRY_RETURNHOME), METHOD, .method = &RosalinaMenu_ReturnToHomeMenu },
            { Lang11_Get(SID_MENU_ROOT_ENTRY_POWOFFRESET), METHOD, .method = &RosalinaMenu_PowerOffOrReboot },
            { Lang11_Get(SID_MENU_ROOT_ENTRY_SYSTEMINFO), METHOD, .method = &RosalinaMenu_ShowSystemInfo },
            { Lang11_Get(SID_MENU_ROOT_ENTRY_CREDITS), METHOD, .method = &RosalinaMenu_ShowCredits },
            { Lang11_Get(SID_MENU_ROOT_ENTRY_DEBUGINFO), METHOD, .method = &RosalinaMenu_ShowDebugInfo, .visibility = &rosalinaMenuShouldShowDebugInfo },
            {},
        }
    };
